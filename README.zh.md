...
- Nix（Flake）：`nix run github:levonk/acryl`（TUI）或 `nix run github:levonk/acryl#acryl-desktop`（桌面 GUI）。也可 `nix profile install github:levonk/acryl` 安装到 Nix profile。Flake 从源码构建，支持 tag pinning（如 `github:levonk/acryl/v0.1.19`）。
