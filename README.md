...
# Run the prebuilt release binary (bundles its own Node runtime)
nix run github:acryldev/acryl#prebuilt
...
The flake exposes `packages.<system>.acryl` (TUI, from source, also `#default`), `packages.<system>.prebuilt` (prebuilt release binary with bundled Node runtime; not available on `x86_64-darwin` because the release does not ship a darwin-x64 CLI tarball — use `#default` there), `packages.<system>.acryl-desktop`, and `devShells.<system>.default`.
