{
  ...
  outputs = { self, nixpkgs, nixpkgs-darwin-legacy, systems, ... }:
    let
      ...
      # v0.1.36 does not ship a darwin-x64 (Intel macOS) CLI tarball —
      # #prebuilt is only exposed on platforms with a release asset.
      # #default (#acryl, from source) works on all platforms including
      # x86_64-darwin.
    ...
    in
    {
      packages = forAllSystems ({ pkgs, system }:
        let
          ...
          # NOTE: hash is lib.fakeHash — the correct hash must be computed
          # by the first CI run. The pnpmDeps FOD times out on machines with
          # slow npm registry connectivity because pnpm 11's fetch-timeout
          # config is not respected inside the nix sandbox. CI runners have
          # fast network and will compute the correct hash on first build.
          ...
          '';
        in
        nixpkgs.lib.optionalAttrs (builtins.hasAttr system prebuiltAssets) {
          ...
          # Only exposed on platforms with a release asset (v0.1.36 does
          # not ship darwin-x64). Use #default or #acryl on platforms
